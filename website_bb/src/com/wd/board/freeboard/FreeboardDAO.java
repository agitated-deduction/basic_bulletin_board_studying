...
public class FreeboardDAO implements BoardDAO{
	...
	public int insert(BoardDTO dto) throws Exception{
		...
		String sql = "insert into freeboard(num, title, writer, contents, reg_date, hit) "
	...
	public int update(BoardDTO dto) throws Exception{
		Connection conn = DBConnector.getConnect();
		String sql = "update freeboard set title=?,writer=?,contents=? where num=?";
		...
		int result = pstm.executeUpdate();//2019-06-04 executeUpdate(sql); >> not all variables bound 오류
	...
	public List<BoardDTO> selectList(int s, int e, String searchBy, String search) throws Exception{
		...
		String sql = "select * from "
				...
				+ "(select * from freeboard where "+searchBy+" like ? order by num desc)N) "
				+ "where R between ? and ?";
		PreparedStatement pstm = conn.prepareStatement(sql);
	...
	public int getTotal (String searchBy, String search)throws Exception{
		...
		Connection conn = DBConnector.getConnect();
		String sql = "select count(num) from freeboard "
				+ "where "+ searchBy+" like ?";
